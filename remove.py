...
# file2.drop(['types', 'attributes', 'isOriginalTitle', 'language'], axis = 1, inplace = True) #remove types, attributes, isOriginalTitle, language column
...
# file2.drop(file2.loc[file2['region'] == '\\N'].index, inplace = True) #remove region == '\\N' row
...
file3.drop(file3.loc[file3['startYear'] == '\\N'].index, inplace = True) # remove startYear == '\\N' row
file3.drop(file3.loc[file3['runtimeMinutes'] == '\\N'].index, inplace = True) # remove runtimeMinutes == '\\N' row
file3.drop(file3.loc[file3['genres'] == '\\N'].index, inplace = True) # remove genres == '\\N' row
file3.drop(file3.loc[file3['titleType'] != 'movie'].index, inplace = True) # remove titleType != 'movie' row
...
# file4.drop(file4.loc[file4['writers'] == '\\N'].index, inplace = True) # remove writers == '\\N' row
# file4.to_csv("new.title.crew.csv", index = False) # output

# # title.episode.csv
# file5 = pd.read_csv("title.episode.csv") # read title.episode.csv
# file5.drop(file5.loc[file5['seasonNumber'] == '\\N'].index, inplace = True) # remove seasonNumber == '\\N' row
# file5.drop(file5.loc[file5['episodeNumber'] == '\\N'].index, inplace = True) # remove episodeNumber == '\\N' row
...
# file7.drop(file7.loc[file7['numVotes'] < 500].index, inplace = True) #remove numVotes < 1000 row
